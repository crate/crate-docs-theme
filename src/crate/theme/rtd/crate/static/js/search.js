...
fetch(`https://www.googleapis.com/customsearch/v1?key=${apikey}&cx=${cx}&q=${input}`).then(response => response.text()).then(text => {
    ...
    let txt = '';
    txt += '<li style="background-image: none !important;padding: 0 !important;">';
      txt += '<p style="font-weight: bold;">';
        txt += 'Your search did not match any documents. Please make sure that all words are spelled correctly.';
      txt += '<p>';
    txt += '</li>';
